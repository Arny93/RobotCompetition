...
//POSITION
float pos_X = 0;//finisher 10
float pos_Y = 0;//finisher 190
...
void *thRobot(void *arg){
    ...
    if(whoAmI == 0){
        ...
        leaveBiggerStadium(motors,sensors,0+(int)side); //As a beginner
        whoAmI = 2;
        sendNext = 0;
    }
    else{
        while(!receiveNext){
           printf("Wait the other robot finish\n");
        }
        printf("The other robot has finished\nThe robot is a finisher\n");
        leaveBiggerStadium(motors,sensors,2+(int)side); // As a finisher
        finitoFin = 0;
        receiveNext = 0;
...
int main()
{
    ...
    //THREAD OF ROBOT AND BLUETOOTH
    pthread_create(&blue,NULL,thBluetooth,NULL);
    pthread_create(&robot,NULL,thRobot,NULL);

    //WAIT THE ROBOT THREAD FINISH
